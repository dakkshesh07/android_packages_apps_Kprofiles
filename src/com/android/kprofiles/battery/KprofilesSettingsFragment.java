...
public class KprofilesSettingsFragment extends PreferenceFragment implements
    ...
    public void onResume() {
        ...
        // Update the enabled state or visibility of the mode preference based on automatic mode setting
        if (FileUtils.fileExists(KPROFILES_AUTO_NODE)) {
            final String autoValue = FileUtils.readOneLine(KPROFILES_AUTO_NODE);
            boolean autoEnabled = autoValue != null && autoValue.equals(ON);
            kProfilesModesPreference.setEnabled(!autoEnabled);
            kProfilesModesInfo.setEnabled(!autoEnabled);
        }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        switch (key) {
            ...
            case KPROFILES_MODES_KEY:
                final String autoValue = FileUtils.readOneLine(KPROFILES_AUTO_NODE);
                if (autoValue != null && autoValue.equals(ON)) {
                    // Automatic mode switching is enabled, do not change mode
                    return false;
                }
